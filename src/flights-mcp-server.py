        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status != 200:
                    print(
                        f"Error fetching CSV: HTTP {response.status}", file=sys.stderr
                    )
                    return {}
                ...
                try:
                    ...
                    print(
                        f"Saved airports to cache file: {AIRPORTS_CACHE_FILE}",
                        file=sys.stderr,
                    )
                except Exception as cache_e:
                    print(
                        f"Warning: Could not save airports cache: {cache_e}",
                        file=sys.stderr,
                    )
...
def search_flights(
    ...
    try:
        ...
        if to_airport not in airports:
            return (
                f"Error: Arrival airport code '{to_airport}' not found in our database."
            )
    ...
    try:
        ...
        flight_data = [
            FlightData(
                date=departure_date, from_airport=from_airport, to_airport=to_airport
            )
        ]
...
def format_flight_results(result, trip_type: str, max_results: int) -> str:
    ...
    for i, flight in enumerate(result.flights[:max_results], 1):  # Limit to max results
        best_tag = (
            " [BEST OPTION]" if hasattr(flight, "is_best") and flight.is_best else ""
        )
    ...
    if len(result.flights) > max_results:
        output.append(
            f"... and {len(result.flights) - max_results} more flight options available."
        )
...
def airport_search(query: str, ctx: Context = None) -> str:
    ...
    if len(matches) > 20:
        result.append(
            f"...and {len(matches) - 20} more. Please refine your search to see more specific results."
        )
...
def get_travel_dates(
    days_from_now: Optional[int] = None, trip_length: Optional[int] = None
) -> str:
    """
...
def get_all_airports() -> str:
    ...
    for code, name in sorted(airports.items())[
        :100
    ]:  # Limit to first 100 to avoid overwhelming
        result.append(f"{code}: {name}")

    if len(airports) > 100:
        result.append(
            f"... and {len(airports) - 100} more airports. Use airport_search tool to find specific airports."
        )
